...
const bottonsIndex = code.indexOf('{/* Botões do Form (Exatamente como o print) */}');
if (bottonsIndex !== -1) {
    const endDivIndex = code.indexOf('</>', bottonsIndex);
    if (endDivIndex !== -1) {
        code = code.substring(0, bottonsIndex) + code.substring(endDivIndex);
    }
}
...
if (code.indexOf('{/* B CONFIRMATION MODAL */}') === -1) {
